window.HDFaceGame = function HDFaceGame(options) {
  ...
  const sketch = (sk) => {
    ...
    function bossDraw() {
      ...
      sk.fill(180); sk.rect(300, 0, 200, 300);
      sk.fill(150); sk.rect(300, 100, 200, 200);
      sk.arc(400, 300, 200, 300, sk.TWO_PI, sk.PI);
      ...
      if (bossState === 'beam_charge') {
        ...
        sk.ellipse(400, 470, glowSize, glowSize);
      }
      sk.fill('#ddcf4f');
      sk.triangle(380, 440, 420, 440, 400, 490);
      ...
      sk.ellipse(330, 350, 50, 70); sk.ellipse(470, 350, 50, 70);

      const orbitR  = 8;
      const lPupilX = 320 + sk.cos(spinAngle) * orbitR;
      ...
      const rPupilX = 480 + sk.cos(-spinAngle) * orbitR;
      ...
      sk.push(); sk.translate(300,150); sk.rotate(flapAngle); drawWing(); sk.pop();
      sk.push(); sk.translate(500,150); sk.scale(-1,1); sk.rotate(flapAngle); drawWing(); sk.pop();
    ...
    function pooBeem() {
      ...
      sk.stroke(80,50,0,beamAlpha);         sk.strokeWeight(beamWidth+6);   sk.line(400+wobble,490,400+wobble,sk.height);
      sk.stroke(139,90,43,beamAlpha);       sk.strokeWeight(beamWidth);     sk.line(400+wobble,490,400+wobble,sk.height);
      sk.stroke(180,130,70,beamAlpha*0.6);  sk.strokeWeight(beamWidth*0.3); sk.line(400+wobble,490,400+wobble,sk.height);
      sk.noStroke();
    }
    ...
    function crossPoopDraw() {
      if (bossState==='cross_warn' && crossWarning.length>0) {
        ...
        sk.text(Math.ceil((CROSS_WARN-stateTimer)/60), 400, 250);
    ...
    function diveDraw() {
      if (bossState==='dive_warn') {
        ...
        for (let i=0;i<3;i++) { const ay=520+i*50; sk.line(400,ay,380,ay-25); sk.line(400,ay,420,ay-25); }
        sk.noStroke();
      ...
      if (bossState==='dive_attack') {
        ...
        for (let i=0;i<6;i++) { const tx=280+i*30; sk.line(tx,bossY+150,tx+sk.random(-5,5),bossY+150-trailLen); }
        sk.noStroke();
      }
    }
    ...
    function checkBossHit() {
      ...
      if (beamActive) {
        if (sk.abs(px-400)<60 && py>490) { hit=true; dmg=BEAM_DAMAGE; }
      }
